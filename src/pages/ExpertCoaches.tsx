...
export default function ExpertCoaches() {
  ...
  const [selectedGuidanceExpert, setSelectedGuidanceExpert] = useState<Expert | null>(null);
  const [guidanceMessage, setGuidanceMessage] = useState('');
  const [guidanceSuccess, setGuidanceSuccess] = useState(false);
  const [filterOpenToHelp, setFilterOpenToHelp] = useState(false);
  ...
  const openGuidanceModal = (expert: Expert) => {
    setSelectedGuidanceExpert(expert);
    setShowGuidanceModal(true);
    setGuidanceMessage('');
    setGuidanceSuccess(false);
  };

  const closeGuidanceModal = () => {
    setShowGuidanceModal(false);
    setSelectedGuidanceExpert(null);
    setGuidanceMessage('');
    setGuidanceSuccess(false);
  };
  ...
  const handleSendGuidance = () => {
    ...
    setTimeout(() => closeGuidanceModal(), 2000);
  };

  const filteredExperts = filterOpenToHelp 
    ? experts.filter(expert => expert.openToHelp)
    : experts;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <label className="flex items-center gap-2 text-sm text-blue-700 font-medium cursor-pointer">
          <input
            type="checkbox"
            checked={filterOpenToHelp}
            onChange={e => setFilterOpenToHelp(e.target.checked)}
            className="accent-blue-600 h-4 w-4 rounded"
          />
          <CheckCircleIcon className="h-4 w-4 text-green-500" /> Show only "Open to Help" experts
        </label>
      </div>
        ...
        {filteredExperts.map((expert) => (
          ...
          >
            ...
            <div className="flex items-start space-x-4">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  {expert.openToHelp && <span className="bg-green-100 text-green-700 px-2 py-0.5 rounded-full text-xs font-semibold">Open to Help</span>}
                </div>
              </div>
            </div>
            ...
            <div className="mt-4 space-y-2">
              ...
              {expert.openToHelp && expert.messageLimit && expert.messagesThisWeek !== undefined && (
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    openGuidanceModal(expert);
                  }}
                  ...
                  disabled={expert.messagesThisWeek >= expert.messageLimit}
                >
                  Ask for Guidance ({expert.messageLimit - expert.messagesThisWeek} left)
                </button>
              )}
            </div>
          </div>
            ...
            <div className="inline-block transform overflow-hidden rounded-lg bg-white text-left align-bottom shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-4xl sm:align-middle">
              <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    ...
                    {selectedExpert.projects.map((project, index) => (
                      <div key={index} className="p-4 border border-gray-200 rounded-lg">
                        ...
                        {project.link && (
                          ...
                          >
                            View Project <ArrowTopRightOnSquareIcon className="h-4 w-4 ml-1" />
                          </a>
                        )}
                      </div>
                    ))}
                ...
                <div className="mt-6">
                  ...
                  <div className="space-y-3">
                    {selectedExpert.reviewList.map((review) => (
      ...
      {showGuidanceModal && selectedGuidanceExpert && (
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="flex min-h-screen items-center justify-center px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            <div 
              className="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity"
            ...
            <div className="inline-block transform overflow-hidden rounded-lg bg-white px-4 pt-5 pb-4 text-left align-bottom shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-md sm:p-6 sm:align-middle">
              <button className="absolute top-3 right-3 text-gray-400 hover:text-gray-600" onClick={closeGuidanceModal}><XMarkIcon className="h-6 w-6" /></button>
              <div className="flex items-center gap-3 mb-4">
                <img src={selectedGuidanceExpert.image} alt={selectedGuidanceExpert.name} className="h-10 w-10 rounded-full" />
                <div>
                  <div className="font-semibold text-gray-900">{selectedGuidanceExpert.name}</div>
                  <div className="text-xs text-gray-500">{selectedGuidanceExpert.title} @ {selectedGuidanceExpert.company}</div>
                </div>
              </div>
              <h3 className="text-lg font-bold text-blue-700 mb-2">Request Guidance</h3>
              {guidanceSuccess ? (
                <div className="text-green-600 font-semibold text-center py-6">Your request was sent! {selectedGuidanceExpert.name} will get back to you soon.</div>
              ) : (
                <>
                  <textarea
